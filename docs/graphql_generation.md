...
and [administration/src-gen/graphql.ts](../administration/src-gen/graphql.ts).
...
1. If schema changed: Generate updated GraphQL schema into [schema.graphql](../frontend/schema.graphql)
   Run "Generate GraphQL specs" in Intellij (or `./gradlew run --args="graphql-export ../specs/backend-api.graphql"` in
   the
   backend folder)

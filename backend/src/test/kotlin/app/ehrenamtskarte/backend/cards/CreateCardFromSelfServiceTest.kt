...
internal class CreateCardFromSelfServiceTest : GraphqlApiTest() {
    ...
    fun `POST returns an error when user entitlements not found in the db`() =
        JavalinTest.test(app) { _, client ->
            ...
            jsonResponse.apply {
                assertEquals(
                    "Error USER_ENTITLEMENT_NOT_FOUND occurred.",
                    findValuesAsText("message").single(),
                )
            }
    ...
    fun `POST returns an error when user entitlements expired`() =
        JavalinTest.test(app) { _, client ->
            ...
            jsonResponse.apply {
                assertEquals(
                    "Error USER_ENTITLEMENT_EXPIRED occurred.",
                    findValuesAsText("message").single(),
                )
            }
    ...
    fun `POST returns a successful response when cards are created`() =
        JavalinTest.test(app) { _, client ->
            ...
            transaction {
                ...
                val userEntitlement = UserEntitlementsEntity.find {
                    UserEntitlements.id eq userEntitlementId
                }.single()

                CardEntity.find {
                    Cards.cardInfoHash eq newDynamicActivationCode.decodeBase64Bytes()
                }.single().let {
                ...
                CardEntity.find {
                    Cards.cardInfoHash eq newStaticVerificationCode.decodeBase64Bytes()
                }.single().let {

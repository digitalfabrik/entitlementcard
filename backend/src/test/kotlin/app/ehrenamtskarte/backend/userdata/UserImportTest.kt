...
internal class UserImportTest : GraphqlApiTest() {
    ...
    fun `POST returns an error response when the auth token is missing`() {
        val response = importUsers(csvFile = null, token = null)
        ...
        assertEquals(
            "Authorization token expired, invalid or missing",
    ...
    fun `POST returns an error response when the auth token is expired`() {
        ...
        val response = importUsers(csvFile = null)
    ...
    fun `POST returns an error response when self-service is not enabled in the project`() {
        ...
        assertEquals(
            "User import is not enabled in the project",
            jsonResponse["message"].asText(),
        )
    }
    ...
    fun `POST returns an error response when multiple files uploaded`() {
        ...
        val request = Request.Builder()
            .url("http://localhost:${backendConfiguration.server.port}/$USER_IMPORT_PATH")
    ...
    fun `POST returns an error response when required columns are missing`() {
        ...
        assertEquals(
            "Missing required columns: [userHash, startDate, endDate]",
            jsonResponse["message"].asText(),
        )
    ...
    fun `POST returns an error response when userHash is not valid`() {
        ...
        assertEquals(
            "Error at line 1: Failed to validate userHash",
            jsonResponse["message"].asText(),
        )
    ...
    private fun importUsers(csvFile: File?, token: String? = "dummy"): Response {

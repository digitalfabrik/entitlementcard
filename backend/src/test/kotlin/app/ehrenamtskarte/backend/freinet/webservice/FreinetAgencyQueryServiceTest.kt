...
internal class FreinetAgencyQueryServiceTest : GraphqlApiTest() {
    ...
    @Test
    fun `POST returns a successful response without information if a region has no agency information`() = JavalinTest.test(app) { _, client ->
    ...
    @Test
    fun `POST returns a successful response with agency information if region has information`() = JavalinTest.test(app) { _, client ->
        val query = createQuery(9, projectId)

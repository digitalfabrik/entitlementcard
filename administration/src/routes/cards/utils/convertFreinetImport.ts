...
const mergeFirstAndLastnameIntoNewColumn = (
  line: string[],
  csvHeader: string[],
  ...
  const fullname =
    !!line[indexFirstName] && !!line[indexLastName]
      ? `${line[indexFirstName]} ${line[indexLastName]}`
      : ''
...
const getCardTypeByFreinetValue = (line: string[], indexCardTypeFreinet: number): string =>
  line[indexCardTypeFreinet].toLowerCase().includes('blau')
    ? BAVARIA_CARD_TYPE_STANDARD
    : BAVARIA_CARD_TYPE_GOLD
 ...
 */
const convertFreinetLineAndHeaders = (
  line: string[],
  csvHeader: string[],
  ...
  if (projectConfig.card.extensionColumnNames[0]) {
    setCardType(
      line,
      csvHeader,
      projectConfig.card.extensionColumnNames[0],

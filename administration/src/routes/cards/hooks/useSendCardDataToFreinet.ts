  ...
  const [sendCardDataToFreinet] = useSendCardDataToFreinetMutation({
    onCompleted: data => {
      ...
      if (successCount === 1 && totalCount === 1) {
        enqueueSnackbar(t('cards:freinetCardDataTransferSuccessMessage'), {
          variant: 'success',
        })
      } else if (successCount > 0) {
        enqueueSnackbar(
          t('cards:multipleFreinetCardDataTransferSuccessMessage', { count: successCount }),
          {
            variant: 'success',
          },
        )
      ...
      if (failedUserIds.length === 1 && totalCount === 1) {
        enqueueSnackbar(t('cards:freinetCardDataTransferFailureMessage'), {
          variant: 'error',
          persist: true,
        })
      } else if (failedUserIds.length > 0) {
        enqueueSnackbar(
          t('cards:multipleFreinetCardDataTransferFailureMessage', {

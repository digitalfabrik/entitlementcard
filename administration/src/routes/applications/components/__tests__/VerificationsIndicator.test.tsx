...
import { PreVerifiedEntitlementType, preVerifiedEntitlements } from '../../utils/preVerifiedEntitlements'
...
describe('VerificationQuickIndicator', () => {
  it('should show only awaiting verifications', () => {
    const { getByTestId } = renderWithOptions(<VerificationIndicator verifications={verificationsAwaiting} />, {
      translation: true,
    })
  ...
  it('should show mixed verifications', () => {
    const { getByTestId } = renderWithOptions(<VerificationIndicator verifications={verificationsMixed} />, {
      translation: true,
    })
  ...
  it('should show only rejected verifications', () => {
    const { getByTestId } = renderWithOptions(<VerificationIndicator verifications={verificationsRejected} />, {
      translation: true,
    })
    expect(getByTestId(`indicator-${VerificationStatus.Verified}`).textContent).toBe(': 0')
  ...
  it('should show only verified verifications', () => {
    const { getByTestId } = renderWithOptions(<VerificationIndicator verifications={verificationsVerified} />, {
      translation: true,
    })
...
describe('PreVerifiedQuickIndicator', () => {
  ...
  it.each(cases)('should show %s label when type is %s', (expectedLabel, type) => {
    const { getByText } = renderWithOptions(<PreVerifiedIndicator type={type} />, { translation: true })

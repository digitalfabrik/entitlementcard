...
const JsonFieldView: FunctionComponent<{
  jsonField: JsonField
  baseUrl: string
  token: string
  key?: number
  ...
  if (props.jsonField.type === 'Array') {
    return (
      <>
        ...
        <div style={{ paddingLeft: '10px', borderLeft: '1px solid #ccc' }}>
          {props.jsonField.value.map((jsonField: JsonField, index: number) =>
            JsonFieldView({
              jsonField,
    ...
    const onClick = () => {
      ...
      fetch(downloadUrl, { headers: { authorization: `Bearer ${props.token}` } })
        .then(result => {
          if (result.status === 200) return result.arrayBuffer()
    ...
    return (
      <p>
        ...
        <Button icon='download' onClick={onClick}>
          Anhang ({props.jsonField.value.fileName})
        </Button>
      </p>
    )
  } else {

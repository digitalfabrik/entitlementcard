...
describe('qrcode', () => {
  describe('protobuf to hex converter', () => {
    it('should return correct hex', () => {
      ...
      const protobufQRCode = create(DynamicActivationCodeSchema, {
        info: cardInfo,
      })
      const qrCode = create(QrCodeSchema, {
        qrCode: {
    ...
    it('should be the same staticVerificationCode after converting back', () => {
      ...
      const protobufQRCode = create(StaticVerificationCodeSchema, { info: cardInfo })
      const qrCode = create(QrCodeSchema, {
        qrCode: {
      ...
      const expectedBinary = encodeQRCode(toBinary(QrCodeSchema, qrCode)).getMatrix().getArray()
      const hex = convertProtobufToHexCode(qrCode)

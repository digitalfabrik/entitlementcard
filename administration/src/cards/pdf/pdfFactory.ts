...
export const generatePdf = async (
  ...
  try {
    ...
    const [fontRegular, fontBold] = await Promise.all([
      getFont(targetDocument, projectConfig.pdf.customFont, StandardFonts.Helvetica),
      getFont(targetDocument, projectConfig.pdf.customBoldFont, StandardFonts.HelveticaBold),
    ])
...
async function getFont(
  ...
  if (typeof fontReference === 'string') {
    ...
    return (
      (await loadEmbeddedFont(targetDocument, fontReference)) ??
...
async function loadEmbeddedFont(
  ...
  try {
    for (const page of targetDocument.getPages()) {
      for (const [_, fontRef] of page.node.normalizedEntries().Font.entries()) {
        ...
        if (fontDescriptor) {
          ...
          const fontFile = targetDocument.context.lookup(
          ...
          if (embeddedFontName?.endsWith(fontName) && fontFile) {
            return PDFFont.of(
              ...
              await CustomFontEmbedder.for(
                fontkit,
                decodePDFRawStream(fontFile as PDFRawStream).decode(),
                embeddedFontName,
              ),
            )
          }

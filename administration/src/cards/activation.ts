...
import {
    ...
    CardGenerationModelInput, Region
} from "../generated/graphql";
import hashCardInfo from "./hash";
import uint8ArrayToBase64 from "../util/uint8ArrayToBase64";
import {ApolloClient} from "@apollo/client";

export async function activateCard(client: ApolloClient<object>, activationCode: DynamicActivationCode, region: Region) {
...
export async function activateCards(client: ApolloClient<object>, activationCodes: DynamicActivationCode[], region: Region) {
    const results = await Promise.all(activationCodes.map(async activationCode => activateCard(client, activationCode, region)))
